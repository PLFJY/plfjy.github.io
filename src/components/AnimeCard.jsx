...
function AnimeCard({ item, eager = false }) {
  ...
  const nativeTitle = item.title?.native || item.note || "Untitled";
  ...
  const accentColor = "#4a7bff";
  const showImage = coverUrl && !hasImageError;
  const href =
    item.url ||
    (item.tmdbType && item.tmdbId ? `https://www.themoviedb.org/${item.tmdbType}/${item.tmdbId}` : "#");
